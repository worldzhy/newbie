...
import {ORDER_BY_ASC_DESC, ORDER_BY_FORMAT} from '@framework/pipes/errors.constants';
  ...
  transform(value: string, metadata: ArgumentMetadata): Record<string, 'asc' | 'desc'> | undefined {
    ...
    try {
      ...
      rules.forEach(rule => {
        ...
        if (!['asc', 'desc'].includes(order.toLocaleLowerCase())) throw new BadGatewayException(ORDER_BY_ASC_DESC);
